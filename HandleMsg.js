...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        if (isCmd && msgFilter.isFiltered(from) && !isGroupMsg) { return console.log(color('[SPAM]', 'red'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname)), aruga.reply(from, ' Maaf anda melakukan spam command. silahkan gunakan command nya Kembali dalam waktu 2-5sec', id) }
        if (isCmd && msgFilter.isFiltered(from) && isGroupMsg) { return console.log(color('[SPAM]', 'red'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname), 'in', color(name || formattedTitle)),aruga.reply(from, 'Maaf anda melakukan spam command. silahkan gunakan command nya Kembali dalam waktu 2-5sec', id) }
        //
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai minna~, Im Funtsu Bot. To find out the commands on this Bot type ${prefix}menu`)
                      })
            } else {
	...
	case 'dewabatch':
        ...
        case 'everyone':
            ...
            break
		case 'simisimi':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf Fitur Ini di Non Aktifkan', id)
			break
		case 'simi':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *F U N T S U  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *F U N T S U  B C* 〙\n\n${msg}`)
            }
            ...
            if(find && find.id === groupId){
                ...
                if(cekuser && isIn !== false){
                    if(isKasar){
                        ...
                        if(denda){
                            await aruga.reply(from, "Jangan badword bodoh\nDenda +5.000\nTotal : Rp"+formatin(denda.denda)+"\nJika denda sudah 30.000 maka member akan di kick", id)
                        }
                        cekuser.map(v => {
                            if (v.denda > 25000) {
                                aruga.removeParticipant(find.id, v.id)
